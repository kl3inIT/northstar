...
- **web** — `cd web && pnpm gen:api` → `web/src/lib/hey-api/` (Hey API
  TypeScript types, Fetch API client, SDK functions, Zod schemas, and TanStack
  Query helpers). Runtime configuration lives in `web/src/lib/hey-api-config.ts`
  and uses Hey's `client.setConfig()` to route generated requests through the
  app's CSRF/session-aware `apiFetch`.
