...
- **web** — `cd web && pnpm gen:api` → `web/src/lib/hey-api/` (Hey API
  TypeScript types, fetch client, and SDK functions). Runtime configuration
  lives in `web/src/lib/hey-api-runtime.ts` and routes generated requests
  through the app's CSRF/session-aware `apiFetch`.
