...
| AI route defaults, runtime gateway CRUD, and secret masking | Automated + runtime | `AiPropertiesTests` pins active-gateway defaults and secret-safe rendering; `AiCredentialCipherTests` pins random AES-GCM envelopes and gateway-bound AAD; `AiRouteSettingsServiceTests` pins default/update/reset precedence. Local API boot validates Flyway V34 and create/list/test/delete against PostgreSQL without returning an API key. |
| Web and mobile model selection | Static + browser | Generated OpenAPI client plus web typecheck; Flutter analyze and Assistant service/ViewModel/widget suites; Playwright screenshots at desktop and 390x844 verify route selection, runtime gateway presets, connection feedback, and the searchable native Model Selector. |
