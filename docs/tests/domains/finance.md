...
| V21-V25 schema plus record/edit/search/delete round trip | Automated | `apps/api/src/test/java/com/northstar/api/finance/FinanceServiceIntegrationTests.java` uses PostgreSQL Testcontainers and validates billing anchors, check-ins, and correction-memory migrations. |
...
| Balance check-in and reconciliation | Automated | `FinanceServiceIntegrationTests` covers first baseline, positive/negative discrepancies, ledger math, ordering, and immutable adjustment rows. |
