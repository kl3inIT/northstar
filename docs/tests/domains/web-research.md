...
| Gateway-routed OpenAI search | Automated | `OpenAiWebSearchProviderTests` verifies that an `OPENAI` gateway and selected model drive the Responses request without exposing its credential to the web settings contract. |
| 9Router search and fetch | Automated | `NineRouterWebProviderTests` verifies normalized `/search` and `/web/fetch` requests, route targets, source/page mapping, and gateway-type rejection. |
...
| Settings desktop/mobile rendering | Runtime | Playwright on 2026-07-12 verified gateway creation and Web research routing at desktop and 390x844, including missing-route state, fixed dialog actions, no overflow, and no console errors. |
