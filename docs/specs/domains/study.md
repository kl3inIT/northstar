...
- Examples, collocations, synonyms, antonyms, easily-confused-word contrast,
  ...
  background job polling carries typed artifact references rather than Base64
  bytes. Those references resolve only through the authenticated, job-scoped
  temporary-artifact endpoint and expire after 30 minutes. The API provider
  bounds each artifact at 16 MiB, total generated bytes at 64 MiB, and both
  artifact entries and active enrichment jobs at 100. Apply deletes the session
  only after its transaction commits; rollback keeps it retryable, while
  discard, failure, expiry, or API restart persist nothing.
