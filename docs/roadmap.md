...
| Study tutor V1 | Done | Capture-first study log with weekly summary and mock trend, vocab SRS on Ebisu recall probability, writing tutor with sourced anchored grading + evaluator loop + error corpus, `/study` page, assistant/MCP tools, weekly review facts. Brief section and bulk imports remain deferred. |
| Vocabulary review V1 | Done | Focused keyboard-first weak-card sessions on `/study`, independent English/Chinese libraries, flat General/IELTS/HSK-style deck scopes, default IPA/pinyin and part of speech, advisory answer checks, learner-owned Ebisu ratings, pronunciation, and explicit preview/apply AI enrichment. Cards keep one memory model when moved; chat remains an optional capture/quiz path and there are still no due dates or synthetic intervals. |
| Vocabulary production and rich enrichment | Done | Optional independent meaning→target memory with deck defaults and per-card overrides, weakest-direction sibling-safe queues, best-effort word formation, provider-routed mnemonic images, and in-API background preview/apply jobs that never persist discarded image data. |
