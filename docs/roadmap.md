...
| Vocabulary audio practice | In progress | Explicit word/example TTS enrichment, applied-audio-first browser fallback, retained pronunciation recordings/history, provider-aware delivery trends, Shadowing, and deterministic Dictation are implemented with clean-schema and live provider verification. A real browser walkthrough remains before consolidation can close. |
