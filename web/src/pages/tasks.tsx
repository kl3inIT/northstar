...
function GroupedViews({
  ...
  const groups: { label: string; tone?: 'danger' | 'muted'; tasks: Task[] }[] = [
    ...
    { label: 'Someday', tone: 'muted', tasks: filteredSomeday },
    {
      label: 'Done',
      tone: 'muted',
      tasks: filteredDated
        ...
        {visible.map((g) => (
          <section key={g.label}>
            ...
            <div className="divide-y">
              {g.tasks.map((t) => (
                <TaskLine key={t.id} task={t} overdue={g.label === 'Overdue'} disciplines={disciplines} />
              ))}
            </div>
          </section>
        ))}
          ...
          <div className="mt-2 space-y-2">
            {g.tasks.map((t) => (
              <Card key={t.id} className="gap-1 p-3">
                <TaskLine task={t} overdue={g.label === 'Overdue'} disciplines={disciplines} compact />
              </Card>
            ))}
          </div>
        </div>
